...
for max_triplets_num in [3]:   
    ...
    if sampling == 'random':
        llm_res_path = './llm_response/MUSIC10K/mpnet_llama3-1/music10k_llm_res_t{}_random.tsv'.format(str(max_triplets_num)) 
        ...
        llm_res_path = './llm_response/MUSIC10K/mpnet_llama3-1/music10k_llm_res_t{}_cluster.tsv'.format(str(max_triplets_num)) 
...
music_embeddings = bert_encode_numpy_music(attr_text_list)   # 4803*768
embeddings = complete_embeddings(music_embeddings, node_id2neigbour_music_id)
save_to_pkl(embeddings, pkl_path)

pkl_path = '{}{}/mpnet_llama3-1/concat_text_mpnet-embeddings_cluster_t10_norm.pkl'.format(root_path, dataset_name)
file_path = './datasets/MUSIC10K/mpnet/music10k_merge_cluster_t10.tsv'  
concat_text_list = get_concat_text_list(file_path)
music_embeddings = bert_encode_numpy_music(concat_text_list)   # 4803*768
embeddings = complete_embeddings(music_embeddings, node_id2neigbour_music_id)
save_to_pkl(embeddings, pkl_path)
'''